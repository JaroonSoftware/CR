...
import { ShoppingCartOutlined, MinusCircleTwoTone } from "@ant-design/icons";
...
const Login = () => {
  ...
  const [itemsOptionChangeSubCategory, setItemsOptionChangeSubCategory] = useState("");
  //Prod Detail
      ...
      .then((res) => {
        ...
        if (status === 200) {
          data.data.forEach((array) => {
            array.img_filename = `${BACKEND_URL_MAIN}/upload_logo/` + array.img_filename;
          });
  ...
  const ProductEcommerce = (ctgy_id) => {
    ...
    if(id === "" || id === undefined){ id = ctgy_id;} else{ id = id}
    if(subctgy_id === "" || subctgy_id === undefined){ subctgy_id = "";} else{ subctgy_id = subctgy_id}
    if(type === "" || type === undefined){ type = "";} else{ type = type}
    let para = {
      ctgy_id : id,
...
// }, [itemsOptionChangeCategory]);
  ...
  const AddProdInCart = () => {
    ...
    if (existingData) {
      ...
      const dataIndex = newData.findIndex(item => item.prod_code === ProdCode && item.size_id === size[0]);
      if (dataIndex !== -1) {
  ...
  const HandleDelProductInCart = (id,size) => {
    ...
    let newData = data.filter(item => item.prod_code !== id || item.size_name !== size);
    console.log(newData)
  ...
  const handleCartAmountChange = (id,size,value) => {
    ...
    .then((res) => {
      ...
      if (status === 200 && sts === '1') {
        ...
        const index = newData.findIndex(item => item.prod_code === id && item.size_id === size);
        if (index > -1) {
        ...
        if (existingData) {
          ...
          // ตรวจสอบว่าข้อมูลที่ต้องการอัปเดตอยู่ใน Array หรือไม่
          const dataIndex = newData2.findIndex(item => item.prod_code === id && item.size_id === size);
          if (dataIndex !== -1) {
            // ถ้าข้อมูลอยู่ใน Array ให้ทำการอัปเดตข้อมูล
            newData2[dataIndex] = { 
  ...
  const showModalCart = () => {
    ...
    if(newData !== null && newData !== undefined) { setItemsCart(newData); }else{ setItemsCart([]); }
    console.log(itemsCart);
     ...
     {ShowCategory && (
      ...
      {ShowProduct && (
        ...
        >
          ...
          <Button style={{
            marginLeft: 5,
            marginBottom: 5,
          }} onClick={BackPage}>ย้อนกลับ</Button>
            <Card
              style={{
                minHeight: 150,
              }}
            >
              ...
              <Form
                ...
                initialValues={{}}
              >
              <Row gutter={[24, 0]} offset={6} className="content-top-content">
                ...
                <Col xs={24} sm={24} md={8} lg={8} xl={4}>
                  ประเภทสินค้า
                  <Form.Item name="prodty_id" rules={[{ required: true, message: 'กรุณาระบุประเภทสินค้า!' }]}  >
                    <Select
                    showSearch
                    style={{ height: 40 }}
          ...
          footer={
            <>
              <Space direction="horizontal" size="middle" style={{ display: 'flex', justifyContent:'end' }} > 
                ...
                {itemsCart.length > 0 && <Button type='primary' onClick={handleshopOk} >สั่งซื้อสินค้า</Button>}
              </Space>
            </>
            }
        >
              ...
              >
                <Row align="middle" gutter={[24, 0]}>
                  <Col align="center" xs={2} sm={2} md={2} lg={2} xl={2}>
                    <Button
                      onClick={() => HandleDelProductInCart(list.prod_code,list.size_name)}
                      type="link"
                  ...
                  <Col align="center" xs={12} sm={7} md={4} lg={4} xl={4}>
                    <InputNumber
                      ...
                      max={99}
                      value={list.amount}
                      onChange={(value) => handleCartAmountChange(list.prod_code,list.size_id,value)}
                    />
                  </Col>
                  <Col align="center" xs={12} sm={7} md={4} lg={4} xl={4}>
                    <p style={{ paddingTop: 15 }}>{list.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} ฿</p>
                  </Col>
                </Row>
              </Card>
              ))}
              <Card hoverable style={{ marginTop: 30 }}>
                <Row align="middle" gutter={[24, 0]}>
                  ...
                  <Col align="right" span={12}>
                    <h3 style={{ paddingTop: 17 }}>{itemsCart.reduce((accumulator, currentValue) => {
      ...
      <Modal
        ...
        footer={
          <>
            <Space direction="horizontal" size="middle" style={{ display: 'flex', justifyContent:'end' }} > 
              {amountProd > 0 && <Button type='primary' onClick={()=>{AddProdInCart()}} >เพิ่มลงตะกล้าสินค้า</Button>}
      ...
      >
        ...
        <Row gutter={[24, 0]}>
          ...
          <Col style={{ paddingTop: 15, }} xs={24} sm={24} md={15} lg={15} xl={15}>
            <Row gutter={[24, 0]} style={{ padding: 30 }}>
              <Col style={{ padding: 10 }} xs={24} sm={24} md={24} lg={24} xl={24}>
              ...
              <Col style={{ padding: 10 }} xs={24} sm={24} md={24} lg={24} xl={24}>
                ...
                <InputNumber
                  ...
                  max={maxProd}
                  defaultValue={0}
                  onChange={(val)=>{setamountProd(val)}}
                />
                </Form.Item>
